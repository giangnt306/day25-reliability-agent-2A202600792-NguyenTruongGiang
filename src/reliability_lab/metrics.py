...
class RunMetrics(BaseModel):
    ...
    def write_csv(self, path: str | Path) -> None:
        """Export metrics as a single-row CSV (spreadsheet / BI friendly).

        The nested ``scenarios`` dict is flattened into ``scenario_{name}``
        columns so every metric is a flat, tabular field.
        """
